...
public class Matrix3f {
    ...
    public static Matrix3f multiply(final Matrix3f m1, final Matrix3f m2) {
        Matrix3f res = new Matrix3f(new float[SIZE][SIZE]);
        for (int arr1rcol = 0; arr1rcol < SIZE; arr1rcol++) {
            for (int arr2row = 0; arr2row < SIZE; arr2row++) {
                float a = 0;
                for (int i = 0; i < SIZE; i++) {
                    a += m1.mat[i][arr1rcol] * m2.mat[arr2row][i];
                }
                res.mat[arr2row][arr1rcol] = a;
            }
        }
        return res;
    }
    ...
    public Vector3f mulVector(final Vector3f v) {
        ...
        for (int col = 0; col < SIZE; col++) {
            for (int row = 0; col < SIZE; col++) {
                res[col] += this.mat[col][row] * arr[row];
            }
        }
    ...
    public void transpose() {
        for (int col = 0; col < SIZE; col++) {
            for (int row = col + 1; row < SIZE; row++) {
                float a = this.mat[col][row];
                this.mat[col][row] = this.mat[row][col];
                this.mat[row][col] = a;
            }
        }
    }
